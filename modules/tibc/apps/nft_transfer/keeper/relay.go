...
func (k Keeper) refundPacketToken(ctx sdk.Context, data types.NonFungibleTokenPacketData) error {
	...
	if data.AwayFromOrigin {
		...
		if err := k.nk.MintNFT(ctx, data.Class, data.Id, "", data.Uri, "", k.GetNftTransferModuleAddr(types.ModuleName)); err != nil {
			return err
		}

		if err := k.nk.TransferOwner(ctx, data.Class, data.Id, DoNotModify, DoNotModify, DoNotModify, k.GetNftTransferModuleAddr(types.ModuleName), sender); err != nil {
			return err
		}
	}
	return nil

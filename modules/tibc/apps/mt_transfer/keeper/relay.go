...
func (k Keeper) SendMtTransfer(
	...
	mt, err := k.nk.GetMT(ctx, class, id)
	if err != nil {
	...
	if awayFromOrigin {
		...
		// burn nft
		if err := k.nk.BurnMT(ctx, class, id, amount, sender); err != nil {
			return err
		}
	}
...
func (k Keeper) refundPacketToken(ctx sdk.Context, data types.MultiTokenPacketData) error {
	...
	if data.AwayFromOrigin {
		...
		k.nk.MintMT(ctx, voucherClass, data.Id, data.Amount, moudleAddr)
		if err := k.nk.TransferOwner(ctx, voucherClass, data.Id, data.Amount, moudleAddr, sender); err != nil {
			return err
		}
	}
	return nil

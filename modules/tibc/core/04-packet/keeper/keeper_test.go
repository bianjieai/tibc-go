...
func (suite KeeperTestSuite) TestGetAllSequences() {
	...
	for _, seq := range []types.PacketSequence{seq1, seq2} {
		suite.chainA.App.IBCKeeper.Packetkeeper.SetNextSequenceSend(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, seq.Sequence)
	}

	sendSeqs := suite.chainA.App.IBCKeeper.Packetkeeper.GetAllPacketSendSeqs(ctxA)
...
func (suite KeeperTestSuite) TestGetAllPacketState() {
	...
	for _, ack := range []types.PacketState{ack1, ack2, ack2dup} {
		suite.chainA.App.IBCKeeper.Packetkeeper.SetPacketAcknowledgement(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, ack.Sequence, ack.Data)
	}

	// set packet receipts
	for _, rec := range expReceipts {
		suite.chainA.App.IBCKeeper.Packetkeeper.SetPacketReceipt(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, rec.Sequence)
	}

	// set packet commitments
	for _, comm := range expCommitments {
		suite.chainA.App.IBCKeeper.Packetkeeper.SetPacketCommitment(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, comm.Sequence, comm.Data)
	}

	acks := suite.chainA.App.IBCKeeper.Packetkeeper.GetAllPacketAcks(ctxA)
	receipts := suite.chainA.App.IBCKeeper.Packetkeeper.GetAllPacketReceipts(ctxA)
	commitments := suite.chainA.App.IBCKeeper.Packetkeeper.GetAllPacketCommitments(ctxA)
...
func (suite *KeeperTestSuite) TestSetSequence() {
	...
	seq, found := suite.chainA.App.IBCKeeper.Packetkeeper.GetNextSequenceSend(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName)
	suite.True(found)
	suite.Equal(one, seq)

	nextSeqSend := uint64(10)
	suite.chainA.App.IBCKeeper.Packetkeeper.SetNextSequenceSend(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, nextSeqSend)

	storedNextSeqSend, found := suite.chainA.App.IBCKeeper.Packetkeeper.GetNextSequenceSend(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName)
	suite.True(found)
...
func (suite *KeeperTestSuite) TestGetAllPacketCommitmentsAtChannel() {
	...
	for i := uint64(1); i < seq; i++ {
		suite.chainA.App.IBCKeeper.Packetkeeper.SetPacketCommitment(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, i, hash)
		expectedSeqs[i] = true
	}
	...
	suite.chainA.App.IBCKeeper.Packetkeeper.SetPacketCommitment(ctxA, path.EndpointA.ChainName, "EndpointBChainName", maxSeq+1, hash)

	commitments := suite.chainA.App.IBCKeeper.Packetkeeper.GetAllPacketCommitmentsAtChannel(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName)
...
func (suite *KeeperTestSuite) TestSetPacketAcknowledgement() {
	...
	storedAckHash, found := suite.chainA.App.IBCKeeper.Packetkeeper.GetPacketAcknowledgement(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, seq)
	suite.Require().False(found)
	suite.Require().Nil(storedAckHash)

	ackHash := []byte("ackhash")
	suite.chainA.App.IBCKeeper.Packetkeeper.SetPacketAcknowledgement(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, seq, ackHash)

	storedAckHash, found = suite.chainA.App.IBCKeeper.Packetkeeper.GetPacketAcknowledgement(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, seq)
	suite.Require().True(found)
	suite.Require().Equal(ackHash, storedAckHash)
	suite.Require().True(suite.chainA.App.IBCKeeper.Packetkeeper.HasPacketAcknowledgement(ctxA, path.EndpointA.ChainName, path.EndpointB.ChainName, seq))
}
